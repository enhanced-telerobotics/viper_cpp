...
viper_ui::viper_ui()
    : Node("viper_ui_node") // Initialize the ROS 2 Node part
{
    init_ros_publisher();
...
void viper_ui::publish_string(const std::string &message)
{
    ...
    RCLCPP_INFO(this->get_logger(), "Published: '%s'", message.c_str());
}
...
uint32_t viper_ui::get_id(viper_usb *pvpr)
{
    uint32_t rv = 0;
    ...
    SEUID_CONFIG *puid = (SEUID_CONFIG *)(resp_pkg + HDR_END_LOC);
...
uint32_t viper_ui::set_unit_quaternion(viper_usb *pvpr)
{
    uint32_t rv = 0;
    const uint32_t HDR_END_LOC = sizeof(SEUCMD_HDR);
    const uint32_t PAYLOAD_END_LOC = sizeof(UNITS_CONFIG);
    uint32_t crc, br;
    ...
    phdr->preamble = VIPER_CMD_PREAMBLE;
    phdr->size = cmd_size - 8; // preamble and size not incl in size
    phdr->seucmd.cmd = CMD_UNITS;
    phdr->seucmd.action = CMD_ACTION_SET;
    UNITS_CONFIG p_payload;
    p_payload.pos_units = POS_METER;
    p_payload.ori_units = ORI_QUATERNION;
...
uint32_t viper_ui::set_ftt_stationary(viper_usb *pvpr)
{
    uint32_t rv = 0;
    const uint32_t HDR_END_LOC = sizeof(SEUCMD_HDR);
    const uint32_t PAYLOAD_END_LOC = sizeof(eFTTMode);
    uint32_t crc, br;

    uint32_t cmd_size = sizeof(SEUCMD_HDR) + PAYLOAD_END_LOC + CRC_SIZE;
    uint8_t *cmd_pkg = new uint8_t[cmd_size];

    uint32_t resp_size = cmd_size + sizeof(eFTTMode);
    uint8_t *resp_pkg = new uint8_t[resp_size];
